package com.usrmgt.spring.master.utils;

public class CONSTANTS {
	
	public static final String dbName = "usermanagerdb";
	public static final String dbtableName = "users";
	public static final String dbColfirstName = "first_name";
	public static final String dbColLastNameDB = "last_name";
	public static final String dbColEmailDB = "email_id";
	public static final String dbColUserID = "userid";
	public static final String dbColR3 = "R3";
	public static final String dbColR4 = "R4";
}
